...
import { HRRR_CONUS_BBOX, SMOKE_RENDERINGS } from "./metadata";
import { buildHrrrGribUrl, buildSmokeTitilerUrl, type HrrrRun } from "./titiler-url";
...
describe("buildSmokeTitilerUrl", () => {
  it("builds a PNG TiTiler bbox URL with one encoded VRT URL", () => {
    ...
    expect(JSON.parse(url.searchParams.get("colormap") ?? "[]")).toEqual(SMOKE_RENDERINGS.density.colormap);
  });
  ...
  it("can render AQI estimate colors from mass-density breakpoints", () => {
    const colormap = JSON.parse(new URL(buildSmokeTitilerUrl({ titilerBaseUrl: "https://titiler.example", run, bbox: HRRR_CONUS_BBOX, rendering: "aqi" })).searchParams.get("colormap") ?? "[]");

    expect(colormap).toEqual(SMOKE_RENDERINGS.aqi.colormap);
    expect(colormap[0][0][1]).toBe(9.1e-9);
    expect(colormap.at(-1)?.[1]).toEqual([126, 0, 35, 255]);
  });
